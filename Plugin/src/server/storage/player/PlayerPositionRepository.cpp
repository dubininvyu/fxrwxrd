...
unsigned int PlayerPositionRepository::createPosition() {
    format fmt = format("INSERT INTO `accounts_pos` (`player_id`) VALUES ('%d')")
                 % player.getUID();
...
bool PlayerPositionRepository::loadPosition() {
    format fmt = format("SELECT * FROM `accounts_pos` WHERE `player_id` = '%d' LIMIT 1")
                 % player.getUID();
...
bool PlayerPositionRepository::savePosition(PersonPosition* position) {
    ...
    format fmt = format("UPDATE `accounts_pos` SET `interior` = '%d', `world` ='%d', `coord_x` = '%f', `coord_y` = '%f', `coord_z` = '%f', `coord_a` = '%f' WHERE `player_id` = '%d' LIMIT 1")
